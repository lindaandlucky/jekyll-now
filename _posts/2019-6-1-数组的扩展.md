...
与rest参数是相反的，Rest 参数接受函数的多余参数，组成一个数组，放在形参的最后

扩展运算符是将一个数组转化为用逗号分隔的参数序列
...
注意： 只有函数调用时，扩展运算符才可以放进圆括号中，否则就会报错
...
关于apply详解      https://blog.csdn.net/lucky541788/article/details/81911349
...
let arr1 = [0, 1, 2];
let arr2 = [3, 4, 5];
arr1.push(...arr2);push方法的参数不能是数组，所以只好通过apply方法变通使用push方法。有了扩展运算符，就可以直接将数组传入push方法。
...
数组是复合的数据类型，直接复制的话，只是复制了指向底层数据结构的指针，而不是克隆一个新的数组

```js
在ES5中只能通过变通的方式复制数组
const a1=[1,2];
const a2 =a1.concat();// 返回原数组的克隆，这样修改a2就不会对a1产生影响
a2[0] =2;
a1//[1,2]
扩展运算符提供了复制数组的简便写法。
const a1=[1,2];
...
关于深拷贝   **https://www.cnblogs.com/echolun/p/7889848.html**
...
在实际应用中，常见的类数组的对象是dom操作返回的nodeList集合以及函数内部的arguments对象
```
...
这个方法可以弥补数组构造函数Array()的不足，因为参数个数的不同，会导致Array()的行为有差异
